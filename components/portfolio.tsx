...
export default function Portfolio({ initialCategory }: { initialCategory?: string }) {
  const router = useRouter()
  const [isMobile, setIsMobile] = useState(false)

  const [hoveredButton, setHoveredButton] = useState<string | null>(null)
  const [clickedButton, setClickedButton] = useState<string | null>(initialCategory ?? null)
  ...
  const [imagePositions, setImagePositions] = useState(initialCategory ? projectPositions : homepagePositions)
  ...
  const handleButtonClick = (buttonName: string) => {
    ...
    // Keep the URL in sync so whatever category is open has a real, shareable
    // link. Categories without a subpage (e.g. the hidden "Product
